...
#aqui empeiza la parte de desarrollo para insertar compras manuales con tickets definidos por el usuario, sin pasar por el proceso normal de compra. Esto es útil para pruebas y para registrar compras que se hicieron por otros medios.
...
def _tickets_faltantes(rifa, tickets):
    db_path = _db_path_for_rifa(rifa)
    if not tickets:
        return []
...
def _reservar_tickets_dev(rifa, tickets):
    db_path = _db_path_for_rifa(rifa)
    if not tickets:
...
def insertar_compra_dev(
    ...
    if not digitos or not str(digitos).isdigit() or len(str(digitos)) != 4:
        raise ValueError("'digitos' debe tener exactamente 4 números.")

    if not tickets:
        raise ValueError("Debes enviar al menos un ticket.")

    _reservar_tickets_dev(rifa, tickets)

    link = f"/{cedula}" if str(rifa) == "1" else f"/2/{cedula}"
    fecha = get_enunciado() if str(rifa) == "1" else get_enunciado2()

    if monto is None:
        precio = get_precio() if str(rifa) == "1" else get_precio2()
        try:
    ...
    if str(rifa) == "1":
        with sqlite3.connect(_db_path_for_rifa(rifa)) as conn:
            cursor = conn.cursor()
            cursor.execute(
                """
        ...
        with sqlite3.connect(_db_path_for_rifa(rifa)) as conn:
            ...
            conn.commit()
        comprador = obtener_comprador_por_cedula2(cedula)
...
def run_dev_cli():
    ...
    insert_parser.add_argument("--rifa", choices=["1", "2"], default="1", help="Base objetivo: 1 o 2")
    ...
    insert_parser.add_argument("--digitos", required=True, help="4 últimos dígitos")
    ...
    if ok:
        print(f"OK: compra insertada en rifa{args.rifa} con tickets {tickets}")
    else:
        print(
    ...
    return True
#aqui termina la parte de desarrollo para insertar compras manuales con tickets definidos por el usuario, sin pasar por el proceso normal de compra. Esto es útil para pruebas y para registrar compras que se hicieron por otros medios.
